...
- 2.1.92：Center 到用户中心的 HTTPS 统一经 `userCenterFetch`，自签兼容开关与 Edge/托盘 `tls_insecure`、OIDC 开关相互隔离。
- 2.1.93：Goal Planner 以 `goal_criteria` 建立任务到成功标准的可追溯映射；Memory Learning 调度失败写入监督事件并默认冷却 15 分钟，手动提取不受冷却影响。
- 2.1.94：Memory Learning 是目标完成后的独立受控阶段，不复用执行期时间/模型调用预算；自动调度以 15 分钟冷却和 3 次失败上限控制资源。
- 2.1.95：Memory Learning 在 Bridge 边界前压缩目标、约束、成功标准、任务和事件上下文，最终 prompt 强制限制为 6000 字符，保持 Edge 协议一致。
- 2.1.97：Center 记忆浏览器以 `/api/steward-memories` 为云端值守记忆事实源，同时保留 `/api/memory/sync` 作为本地 Work 记忆摘要数据源。
- 2.1.98：Memory Learning 在确定性校验后直接写入 approved 生效态；`disable` 将记忆转为 rejected，`restore` 可恢复，去重合并保留人工停用状态以防自动复活。
- 2.1.99：监督 Worker Matcher 以 Bridge Server 内存中的实时 WebSocket 连接为唯一可投递在线依据，避免 sync_clients 陈旧 connected 状态导致假在线投递；Center 通过共享 memory-path 边界将本地记忆根目录置空。
- 2.1.101：Edge Local Session Executor 从 Agent 配置读取受限格式的 `codex_model_provider`，并以命令级 Codex 配置注入新建及恢复会话；Agent 级配置优先于全局配置。
- 2.1.100：Edge HumanWatch Judge 在空回复恢复路径中清除损坏的持久会话绑定，重新执行专用会话 bootstrap 并仅重试一次，避免目标长期卡在 verifying。
- 2.1.96：Memory 生命周期字段保持 nullable 语义，模型 JSON、API 和 SQLite 三层不再把 `null` 强制转换为 Unix 时间 0。
...
- 2.1.77 Activity UI 保持云端 Agent 唯一名称作为选择状态和详情定位，但从 `config.original_name` 派生 API `actor` 参数及本地 session 匹配键，解决 Bridge 索引前缀与 Edge 原始 actor 命名空间不一致。
- 2.1.78 增加 `agent_metrics` 只读 Bridge RPC。单 Agent 诊断/归因按 `client_id + local_agent_id` 请求；评估与 Token 为设备级快照。Center 仅向在线且宣告 capability 的 Edge 请求，通过 `sync_agent_index.original_name -> remote_name` 恢复云端唯一身份。
- 2.1.79 安全审计聚合不得被同步 OS 扫描长时间阻塞。扫描在进程内保留 60 秒快照，系统命令超时最长 1 秒；修复 API 显式绕过缓存。
- 2.1.80 增加 `work_search` 和 `standup_snapshot` 只读 Bridge RPC。搜索条件在 Edge SQL 层过滤，站会按日期范围返回 Agent、任务和活动计数；Center 仅聚合当前 Workspace 允许的在线客户端。
- 2.1.81 Center 在 Edge `hello` 后立即关闭同 `client_id` 的旧 socket 并失败其待定 RPC；Edge `onclose` 仅在关闭 socket 仍是当前 `state.ws` 时才清理状态和安排重连。
- 2.1.82 将 Work 任务写操作接入既有 Cloud Relay / Local Mailbox。Center `POST /api/tasks/local-mutations` 只生成带幂等键和乐观版本的 `work.task.mutation.requested`，Bridge 仅负责唤醒；Edge 通过 pull/lease 落 inbox，执行本地事务后由 outbox ACK/fail。`local_message_executions` 保证重启和重复投递不重复执行，任务投影失效事件负责把最终本地事实刷新到云端 UI。
- 2.1.83 增加 `work_projection_snapshots` 作为可重建展示缓存，主键为 `workspace_id + client_id + kind(tasks|activities)`。实时 Bridge 快照成功后原子 upsert；断线或 RPC 失败时投影层读取 7 天内快照并把每条事实改标为 `local_snapshot/stale`。搜索和站会只对未成功实时查询的客户端使用该回退，避免实时与旧数据重复。
- 2.1.84 Human Watch judge 结果通过幂等 `session.continue.requested` 进入 Cloud Relay / Local Mailbox；Edge CLI 完成 ACK 是 `intervention_completed=success` 和 watch event resolved 的唯一边界。
- 2.1.85 Human Watch judge 输出结构化 action/reply/reason/risk；Center 在入队前使用独立安全分类器复核 Worker 最新问询。`escalate_human` 或 high/critical 决策会保留 visible 高优先级事件并写入 `intervention_skipped`，不创建续话消息；旧自定义判官纯文本输出仍按 normal reply 兼容。
- 2.1.86 编排器在 judge 前查询 active event dedupe key，pending/visible/claimed 存在时终止重复决策。Edge 自动 provision 直接调用自带串行保护的 provision 函数，外层只负责生命周期通知，不再套第二层同 key 锁。
- 2.1.87 中心审计读模型批量关联 `human_watch_interventions.message_id/correlation_id`、`edge_messages` 及消息 payload 中的 `human_watch_event_id`。写入链路和状态机保持不变，UI 通过单一 API 读取触发、排队、ACK、Worker 回复与失败证据，避免 N+1 查询和人工联表。
- 2.1.88 `pollActiveBindings` 在任何 session/Bridge 可用性判断前调用中心侧自动停止检查；`max_runtime_seconds` 等安全阀只依赖 binding 与中心审计数据，离线 Worker 不再绕过到期停用。停用后本轮不得读取 transcript、调用 judge 或创建可靠消息。
- 2.1.89 值守 judge 采用双提示架构：Center 生成完整提示供 CLI 可靠回退，同时生成最多 1600 字符的快速提示，经 Bridge 发送给 Edge。Edge 从 Codex TOML 结构化解析当前 provider/base URL/env key，快速调用只发送同一 provider，3.5 秒失败后回退原有绑定会话；成功用量写入本地 `token_usage`。快速与回退结果继续经过中心危险操作复核、事件去重和可靠 mailbox。
- 2.1.90 快速提示明确值守是用户代理：低风险二选一缺少偏好时直接选择合理选项，禁止继续等待；安全分类、可靠投递和 CLI 回退架构不变。
- 2.1.91 主动求助链路补齐 MCP annotations，并在 Center 同步 assist 边界解析结构化 decision；reply/ask_worker 仅续写最终文本，escalate_human 保持人工事件。
- 快照不改变本地主数据权属，不接受写操作，不保存 Token/文件/会话全文。Workspace 查询、Client 索引过滤、4MiB 上限和 7 天清理共同约束租户隔离与资源占用；重连成功的 client ID 会排除同客户端旧快照。
- Token 快照在 Edge 合并本地数据库、文件和 CLI 会话存储；Center 为记录 ID/session ID 增加设备命名空间，内存合并后复用既有成本计算，不将本地 Token 明细写入中心 SQLite。
...
- 独立 Verifier 只接受结构化独立证据引用。`steward_memories` 和 `steward_memory_usage` 保存受控记忆及使用效果；approved 表示生效、rejected 表示人工停用，仅生效记忆可注入计划、监督、验收和 Reactive Watch，当前会话与安全策略始终优先。
- 2.1.102：目标创建入口通过值守 Agent 同步索引解析 `client_id + local_agent_id`，与面向 Worker 的人工值守绑定解耦。
