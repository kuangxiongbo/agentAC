...
| 适用产品版本 | 2.1.102 |
| 更新时间 | 2026-08-07 |
| 核心来源 | 历史 PRD、人工值守 PRD、值守目标监督与受控记忆 PRD、上游 v2.2.0 安全回迁评估、架构设计、测试报告、2.0.3～2.1.83 release notes、当前代码实现 |

本文档是 Agent 指挥仓的唯一主需求文档。历史专题 PRD、架构设计、任务拆解、测试用例和发布说明只作为证据来源；产品最新完整需求以本文档为准。

- 2.1.92：用户中心使用自签 HTTPS 时，Center 可通过 `MC_USERCENTER_TLS_INSECURE=1` 仅放宽 `USER_CENTER_API_URL` 请求，不影响其他 HTTPS；默认保持严格校验。
- 2.1.93：值守目标计划的每个任务必须声明负责的成功标准，计划必须覆盖全部标准；目标完成后的记忆提取失败必须留痕并有界重试，避免静默失败和模型额度浪费。
- 2.1.94：完成后记忆学习与目标执行阶段的 deadline/runtime/model-call 预算解耦；自动失败最多尝试 3 次且间隔 15 分钟，避免旧完成目标永久无法学习或无界消耗。
- 2.1.95：完成目标的记忆提取上下文按字段确定性裁剪，发送给 Edge 值守判断的 prompt 不超过 6000 字符协议上限，避免大型目标无法形成记忆。
- 2.1.97：服务端记忆页直接展示云端值守记忆库，客户端 Work 记忆摘要作为独立次级区域，不再把“无客户端节点”误报为“无记忆”。
- 2.1.98：值守记忆通过安全和作用域校验后默认自动生效，操作员可停用或恢复；已停用的同内容记忆即使再次学到也不会被自动复活。
- 2.1.99：监督任务只能分派给存在实时 Bridge WebSocket 的 Edge Worker，同步心跳不得作为在线替代；Center 模式不读取本地 OpenClaw 记忆目录。
- 2.1.101：值守 Agent 可配置 `codex_model_provider`，Edge 启动或恢复 Codex 会话时必须使用该 Agent 绑定的中转配置，避免继承不适用的全局 provider。
- 2.1.100：值守 Judge 的持久 CLI 会话返回空回复时，Edge 必须失效旧绑定、自动重建专用会话并重试一次；重建后仍失败时停止自动重试并转人工。
- 2.1.96：候选记忆的空过期时间保持为 NULL，审核接口可显式清除历史错误的零过期时间，保证获批记忆可以进入检索。
...
- 2.1.42 受控记忆：记忆分 preference/fact/episode/procedure；2.1.98 起自动提炼结果校验后直接进入 approved（生效），并支持人工停用、恢复和防止重复学习复活。检索受作用域、条数、字符数、有效期和效果反馈约束。
...
- 2.1.74 Work 主详情一致性：Agent 主详情和任务、活动、文件、SOUL、记忆标签必须共用统一身份解析；旧 ID 重连后页面头部也必须显示实时 Bridge 身份和本地配置。
- 2.1.75 全局任务投影：中心任务看板和 Dashboard 必须在 Edge 在线时展示本地 Runtime 的实时任务与状态统计，并与云端监督控制任务去重合并。本地投影任务在第 5 项可靠写回完成前保持只读，不得将编辑、删除、评论、评审或拖拽状态变更误写到云端同 ID 任务。
- 本地任务新建、更新、删除或状态变化后，Edge 必须发送轻量失效通知，中心清理短期快照并通过 SSE 触发任务页与 Dashboard 重取；通知不上传任务正文。
- 2.1.76 全局活动投影：中心活动流必须合并云端控制事件、本地 Runtime 活动记录和 CLI 会话里程碑，并按统一时间线排序。每条本地事件必须显示 `local_runtime`、设备标识和稳定投影 ID，不得复制为云端主数据。
- 活动类型、Agent、实体、时间和客户端筛选必须同时作用于云端与本地来源；活动统计必须包含在线 Edge 的近期事件。旧 Runtime、Bridge 超时或断线时继续展示云端控制事件，并明确 `local_live=false`，不得伪造本地无活动。
- Edge 持续输出期间活动失效通知最多每秒一次；Center 快照短期缓存且单客户端最多读取 1000 条事件。资源有界优先于无限历史同步，超过上限必须返回 `truncated=true`。
- 2.1.77 Agent 活动筛选必须把云端唯一显示名映射回 `config.original_name` 后查询本地 actor，不能用带 `client_id-` 前缀的镜像名直接过滤而误报无活动。
- 2.1.78 Work Agent 的诊断、归因、评估和 Token/成本指标必须以 Edge 本地数据为权威源，Center 只做实时读取、合并和展示，不得用云端空库统计伪装本地 Work 指标。
- Center 成本页面的 `stats/trends/agent-costs/task-costs/session-costs/by-agent` 必须基于同一份标准化 Token 明细；Edge 离线时保留云端值守 Agent 数据，不缓存或伪造 Work 指标。
- 2.1.79 安全审计页必须在可控时间内展示 Agent 评估投影；基础设施扫描结果可缓存 60 秒，单条系统命令最长 1 秒，修复后必须强制复检。
- 2.1.80 全局搜索和站会必须合并在线 Edge 的 Work 任务、活动和 Agent 事实，显示 `local_runtime`、稳定投影 ID 及云端唯一 Agent 名；Edge 离线不得伪造本地空结果。
- 2.1.81 同一 `client_id` 在 Center 同时只能保留最新 Bridge 连接；Edge 旧 socket 延迟关闭不得清空新连接状态或触发重复重连。
- 2.1.82 Work 任务可靠写回：云端对 `local_runtime` 投影任务的更新、状态拖拽和删除必须创建 `work.task.mutation.requested` 可靠消息，不得直接修改云端同 ID 记录。本地 Runtime 校验客户端、Workspace、本地任务 ID、允许字段和 `expected_updated_at` 后，在 SQLite 事务内写入任务及活动，再 ACK；版本冲突、越权字段和不存在任务必须 fail 且不修改本地数据。
- 重复 `idempotency_key` 不得重复执行；Edge 离线时消息保持 pending，上线后通过 pull/lease 恢复。云端接受排队不等于写回完成，最终状态以 Edge ACK/fail 和实时任务投影为准。
- 2.1.83 Work 离线连续性：Center 必须保存每个 Workspace/客户端最近一次成功的有界任务和活动快照，Edge 断线或 RPC 超时时继续展示但明确标记 `local_snapshot`、`stale=true`、`snapshot_at` 和 `local_live=false`。离线快照可用于任务看板、Dashboard、活动、搜索和站会，但必须保持只读。
- 2.1.84 值守自动代答必须走可靠 mailbox；入队不是成功，只有 Edge 完成原 Worker 会话续写并 ACK 后才关闭事件和记录成功，所有证据必须关联同一消息 ID。
- 2.1.85 值守判官必须输出 `reply|ask_worker|escalate_human` 结构化决策；普通确认、选择和可补充信息可自动代答，生产变更、删除、提权、密钥处理及无法由上下文确定的业务决策必须保留待处理事件并转人工，不得进入 Worker mailbox。
- 2.1.86 同一 binding/session/fingerprint 已有活动 watch event 时不得重复调用 judge 或入队；新建 Edge Agent 的自动会话初始化只能获取一次 Agent 串行锁，不得因嵌套获锁永久停在 provisioning。
- 2.1.87 值守干预记录必须直接展示值守事件、可靠消息、关联标识、投递状态、尝试次数、触发至 ACK 时长、Worker 后续回复和失败原因；操作者不得再依赖手工联表判断自动代答是否真实完成。
- 2.1.88 值守 binding 的运行时长、成功次数和限流次数自动停止属于中心安全策略，必须在每轮轮询中先于 Worker session 与 Edge Bridge 在线检查执行；Edge 离线不得阻止到期 binding 自动停用。
- 2.1.89 高置信 Worker 等待问询必须由 transcript 事件在亚秒级触发值守判断。Edge 优先使用当前 Codex 配置的同一 OpenAI 兼容 provider 执行不超过 1600 字符的结构化快速判断，保留 Worker 上下文、受控记忆与安全规则；失败或超时回退完整值守 CLI 会话。最终是否达到 5 秒只按值守回复进入原 Worker transcript 的真实时间验收。
- 2.1.90 对简洁版/详细版、颜色、格式等低风险二选一，值守必须代替用户给出一个合理选择，不得回复继续等待或把同一问题退回用户；危险操作和只能由业务负责人决定的信息仍转人工。
- 2.1.91 受管 Worker 的 `mc_create_watch_event` 必须可在非交互模式自动调用；同步 assist 只把结构化 decision 的 `reply` 写入 Worker，禁止透传 JSON；`escalate_human` 不代发。
- 快照最长保留 7 天、单份序列化数据不超过 4MiB；重连成功后实时 `local_runtime` 必须立即替换旧快照。旧客户端从未宣告对应 capability 且没有历史成功快照时只显示云端数据，不得伪造空的本地事实。
...
- 2.1.102：新建监督目标时直接选择同步到中心的值守 Agent，不依赖 Worker 人工值守绑定是否启用。
