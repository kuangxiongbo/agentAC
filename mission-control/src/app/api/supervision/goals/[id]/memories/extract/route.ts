...
export async function POST(
  ...
  try {
    return NextResponse.json(await extractStewardMemoryCandidates({
      goalId: id,
      workspaceId: auth.user.workspace_id ?? 1,
    }))
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Memory extraction failed' }, { status: 400 })
  }
}
