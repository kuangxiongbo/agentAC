...
export function MetricCardsWidget({ data }: { data: DashboardData }) {
  ...
  return (
    <section className="grid grid-cols-2 xl:grid-cols-5 gap-3">
      ...
      <MetricCard label={t('metricAgentCapacity')} value={onlineAgents} total={dbStats?.agents.total ?? agents.length} subtitle={t('metricAgentSignalingSubtitle')} icon={<AgentIcon />} color={onlineAgents > 0 ? 'green' : 'purple'} />
