...
export function SupervisionPanel() {
  ...
  if (!centralMode) return <div className="p-6 text-sm text-muted-foreground">目标仅在服务端模式可用。</div>
  return (
    <div className="flex h-full min-h-0 flex-col">
      <div className="flex h-14 shrink-0 items-center justify-between border-b border-border px-4 md:px-6">
        <div>
          <h1 className="text-base font-semibold text-foreground">目标</h1>
...
function GoalDetail({ goalId, onChanged }: { goalId: string; onChanged: () => Promise<void> }) {
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 p-4 md:p-6">
      ...
      <section>
        <h3 className="mb-2 text-sm font-semibold">目标事件</h3>
...
function MemoryWorkspace() {
  ...
  const action = async (memory: Memory, name: 'restore' | 'disable' | 'expire') => { setBusy(memory.id); try { await api(`/api/steward-memories/${memory.id}/actions`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ action: name }) }); await load() } catch (err) { setError(err instanceof Error ? err.message : '操作失败') } finally { setBusy(null) } }
  const statusLabels: Record<string, string> = { approved: '已生效', rejected: '已停用', expired: '已过期', candidate: '历史候选' }
  return <div className="min-h-0 flex-1 overflow-y-auto p-4 md:p-6"><div className="mx-auto max-w-6xl"><div className="mb-4 flex items-center justify-between"><div className="flex gap-1">{['approved', 'rejected', 'expired', 'candidate'].map((item) => <button key={item} onClick={() => setStatus(item)} className={`h-8 px-3 text-xs ${status === item ? 'border-b-2 border-primary text-foreground' : 'text-muted-foreground'}`}>{statusLabels[item]}</button>)}</div><span className="text-xs text-muted-foreground">{memories.length} 条</span></div>{error ? <p className="mb-3 text-xs text-rose-400">{error}</p> : null}{loading ? <Loader variant="panel" label="加载记忆" /> : <div className="divide-y divide-border border-y border-border">{memories.map((memory) => <article key={memory.id} className="grid gap-3 py-4 md:grid-cols-[150px_1fr_auto]"><div><div className="text-xs font-medium">{memory.category}</div><div className="mt-1 font-mono text-[10px] text-muted-foreground">{memory.scope_type}:{memory.scope_id}</div><div className="mt-2 text-[10px] text-muted-foreground">置信度 {Math.round(memory.confidence * 100)}%</div></div><div><h3 className="text-sm font-medium">{memory.summary || memory.content.slice(0, 80)}</h3><p className="mt-1 whitespace-pre-wrap text-xs text-muted-foreground">{memory.content}</p><p className="mt-2 text-[10px] text-muted-foreground">来源 {memory.source_refs.length} · {dateTime(memory.updated_at)}</p></div><div className="flex items-start gap-1">{memory.status === 'approved' ? <Button size="xs" variant="outline" disabled={busy === memory.id} onClick={() => void action(memory, 'disable')}>停用</Button> : null}{memory.status === 'rejected' ? <Button size="xs" variant="success" disabled={busy === memory.id} onClick={() => void action(memory, 'restore')}>恢复</Button> : null}{memory.status === 'candidate' ? <Button size="xs" variant="success" disabled={busy === memory.id} onClick={() => void action(memory, 'restore')}>生效</Button> : null}{memory.status === 'approved' ? <Button size="xs" variant="ghost" disabled={busy === memory.id} onClick={() => void action(memory, 'expire')}>过期</Button> : null}</div></article>)}{memories.length === 0 ? <p className="py-10 text-center text-xs text-muted-foreground">暂无记录</p> : null}</div>}</div></div>
}
