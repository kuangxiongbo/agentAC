...
import { getMainAgentRuntimeMeta, type MainAgentRuntimeId } from './runtime-agents'
...
function buildRuntimeConfig(runtime: ReturnType<typeof detectAllRuntimes>[number]) {
  const meta = getMainAgentRuntimeMeta(runtime.id as MainAgentRuntimeId)
  return {
    ...
    install_supported: meta?.installSupported ?? false,
