...
export function useWebSocket() {
  ...
  const getGatewayErrorHelp = useCallback((message: string): string => {
    ...
    if (
      ...
      return 'Gateway requires device identity. Open E-Agent-Center via HTTPS (or localhost), then reconnect so WebCrypto signing can run.'
    }
