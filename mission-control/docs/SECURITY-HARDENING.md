...
Comprehensive security hardening guide for E-AgentCenter and OpenClaw Gateway deployments.
...
E-AgentCenter uses a host allowlist in production:
...
E-AgentCenter sets these headers automatically:
...
E-AgentCenter acts as the mothership for your OpenClaw fleet. The installer automatically checks and repairs common OpenClaw configuration issues.
...
- Audit third-party skills before installing (E-AgentCenter's Skills Hub runs automatic security scans).
...
- Use E-AgentCenter's diagnostics API to monitor gateway health.
...
[Reverse Proxy (Caddy/nginx) + TLS]
  |
[E-AgentCenter :3000] ---- [SQLite .data/]
  |
...
- E-AgentCenter listens on localhost or a private network
...
E-AgentCenter logs a security event (`global_api_key_used`) every time the global API key is used. Monitor these in the audit log:
