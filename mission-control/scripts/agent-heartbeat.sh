...
# E-AgentCenter Phase 3: Agent Heartbeat Script
...
# Check if E-AgentCenter is running
check_mission_control() {
    if ! curl -s "$MISSION_CONTROL_URL/api/status" > /dev/null 2>&1; then
        log "ERROR" "E-AgentCenter not accessible at $MISSION_CONTROL_URL"
        return 1
    fi
    return 0
}
...
send_wake_notification() {
    ...
    # Format wake message
    local wake_message="🤖 **E-AgentCenter Heartbeat**\n\n"
    ...
    wake_message+="Use E-AgentCenter to view details: $MISSION_CONTROL_URL\n\n"
...
main() {
    ...
    if ! check_mission_control; then
        log "ERROR" "Aborting: E-AgentCenter not accessible"
        exit 1
    fi
...
case "${1:-}" in
    --help|-h)
        echo "E-AgentCenter Agent Heartbeat Script"
        echo ""
        ...
        echo "  MISSION_CONTROL_URL  E-AgentCenter base URL (default: http://localhost:3005)"
        echo ""
        exit 0
        ;;
    *)
