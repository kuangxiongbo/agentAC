...
### FR-V2-000 客户端连接与派生数据清理

系统中的客户端连接必须作为客户端镜像智能体、会话快照、Bridge 索引缓存等派生数据的根对象。

删除连接时必须满足以下规则：

1. 服务状态里只允许对 `disconnected` 客户端显示删除动作。
2. 删除动作必须使用平台统一模拟确认弹窗，不允许使用浏览器原生 `window.confirm / window.prompt / window.alert`。
3. 用户确认删除后，平台必须同时清理：
   - `sync_clients` 客户端记录
   - `sync_agent_index` 对应客户端缓存索引
   - `source='client'` 且 `node_id=client_id` 的镜像智能体
   - `sync_sessions` 中该客户端的会话快照
4. 若该客户端仍存在 human-watch 绑定，则必须阻止删除，并明确提示“先解绑再删”。
5. 删除连接不应影响其他客户端的数据，也不应删除手工创建的本地智能体。

补充展示规则：

- 服务端主列表应只展示在线或离线客户端下的智能体。
- 没有任何客户端归属的历史 `client / bridge / bridge_index` 镜像不应长期占位在主列表中，应自动从主列表收敛。
- 只有真正属于客户端的数据，才应随着客户端删除而被连带清理。
...
服务端与客户端所有需要用户确认、输入确认串、危险删除、审批备注输入、手动回复输入的交互，必须使用统一的应用内模拟弹窗/对话框组件，不允许直接调用浏览器原生弹窗。
...
1. 不允许在生产 UI 中直接使用：
   ...
   - `window.prompt`
   - `window.alert`
2. 删除客户端、删除智能体、解除值守绑定、危险操作确认都必须使用统一确认模态框。
3. 失败原因必须在 UI 中明确展示，不能只写入 `console.error`。
4. 服务端和客户端实现都必须一致，不得一边使用模拟弹窗、一边继续保留原生弹窗。

客户价值：

- 确认流程一致，避免浏览器原生弹窗导致样式不统一、信息不完整、移动端和桌面端体验割裂。
- 可以展示更明确的影响范围，例如“删除连接将同时删除镜像智能体与会话快照”。
...
1. **客户端同步智能体**
   - 来源：`source='client'`
   - 定义：由本地 Web / 托盘 / Edge 客户端同步上来的镜像智能体
   - 规则：
     - 服务端不允许把它当作独立主数据随意删除
     - 若用户删除客户端连接，则连带清理这类镜像智能体
     - 若客户端重新连接，同名/同 `local_agent_id` 的镜像可以重新同步回来

2. **客户端在线时的实时智能体**
   - 来源：`bridge_index`
   - 定义：Bridge 在线时的实时只读索引
   - 规则：
     - 服务端不支持直接删除
     - 应由本地客户端删除后再同步消失

3. **值守智能体（human-watch steward）**
   - 定义：服务端创建并下发到本地的反向管理智能体
   - 规则：
     - 服务端是主控入口
     - 服务端在线连通客户端时，值守删除应优先走服务端下发到本地的删除动作
     - 客户端未连上服务端时，本地允许删除值守，但重新连接后若服务端仍保留该值守定义，它可以再次同步回来
     - 服务端删除值守前必须处理关联绑定

4. **本地手工创建/导入智能体**
   - 定义：非客户端镜像、非 Bridge 索引、非值守的本地主数据
   - 规则：
     - 可由所属端直接删除
     - 不应随着客户端连接清理而误删
...
- 服务端删除连接时，不会误删本地主数据智能体。
- 客户端在线时，普通客户端同步智能体在服务端不可直接删除。
- 值守智能体删除入口与普通客户端同步智能体删除入口明确区分。
