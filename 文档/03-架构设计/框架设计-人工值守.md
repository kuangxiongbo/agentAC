...
- Memory：candidate 默认不可检索；approved 记忆受作用域、有效期、字符数和使用结果约束。自动提取失败写 `memory_candidates_extraction_failed`，相同目标默认冷却 15 分钟、最多自动失败 3 次；完成后学习不复用执行期预算，手动提取可立即执行；2.1.95 起按字段裁剪并保证 Bridge judge prompt 不超过 6000 字符。
