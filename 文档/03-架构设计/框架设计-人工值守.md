...
在中心服实现租户级「人工值守」控制面：边缘创建值守 Agent、中心配置监视绑定、中心编排代发；**每次判定与介入必须写入 `human_watch_interventions` 留痕**。V1.4 增加平台级权限请求与选项决策闭环，解决“只能回复消息、不能代替选择确认项”的问题。V1.6 增加记忆辅助判断，值守 Agent 在回复前可读取受限检索片段，但不能把记忆当作审批事实源。V1.7 持久化 Worker `session_kind`，避免绑定存在但编排阶段无法定位本地会话类型。V2.2 增加 5 秒小范围验证触发、内容化确认回复和自动停止，保证值守 Agent 快速介入但不会无限循环。
