...
在中心服实现租户级「人工值守」控制面：边缘创建值守 Agent、中心配置监视绑定、中心编排代发；**每次判定与介入必须写入 `human_watch_interventions` 留痕**。V1.4 增加平台级权限请求与选项决策闭环，解决“只能回复消息、不能代替选择确认项”的问题。V1.6 增加记忆辅助判断，值守 Agent 在回复前可读取受限检索片段，但不能把记忆当作审批事实源。
