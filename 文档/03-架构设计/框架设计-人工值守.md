...
- 2.1.87 起干预记录 Tab 直接展示值守事件、message/correlation ID、mailbox 终态与尝试次数、触发到 ACK 时间线、Worker 后续回复及失败原因；证据由中心批量关联读模型提供，不改变值守写入链路。
- 2.1.88 起中心轮询先对全部启用 binding 执行自动停止，再检查 session 与 Bridge。运行时长安全阀不依赖 Edge 在线；命中后本轮立即跳过 transcript、judge 和消息投递。
- 2.1.89 起 transcript 更新事件仅防抖 100ms；Center 同时生成有界快速 judge 提示和完整回退提示。Edge 从当前 Codex provider 配置执行快速结构化判断，3.5 秒超时或异常自动回退原绑定 CLI session；快速调用 Token 写入本地 `token_usage`，回复仍须通过中心安全复核和可靠 mailbox。
...
- Planner：值守生成结构化 DAG，中心校验 logical key、依赖、风险、预算和验收标准；2.1.93 起每个任务以 `goal_criteria` 映射目标成功标准，保存前拒绝未知 ID 或未覆盖标准。
...
- Memory：2.1.98 起自动提炼在安全、作用域和内容校验后直接进入 approved（生效）；rejected 表示人工停用，可 restore 恢复，后续相同证据只追加来源而不自动复活。生效记忆受作用域、有效期、字符数和使用结果约束。自动提取失败写 `memory_candidates_extraction_failed`，相同目标默认冷却 15 分钟、最多自动失败 3 次；完成后学习不复用执行期预算，手动提取可立即执行；2.1.95 起 Bridge judge prompt 不超过 6000 字符；2.1.96 起 `expires_at` 保持 nullable 语义。
- Dispatch：2.1.99 起只有 Bridge Server 确认存在实时 Edge WebSocket 时才能匹配 Worker；`sync_clients` 的近期心跳仅用于展示和同步健康，不能作为可投递证据。Center 模式的 `MEMORY_PATH` 为空，不扫描宿主机 OpenClaw 路径。
...
### 8.8 值守 Judge 空回复会话自动恢复（2.1.100）

Codex CLI 的已绑定专属会话可能在上游重连异常后正常退出但不产生 assistant 消息。该结果不是有效判断，也不会触发仅覆盖异常抛出的旧恢复路径。Edge 在 transcript 恢复后仍确认 Judge 为空时，执行有界恢复：失效当前值守 Agent 的专属会话绑定，将会话状态重置为 `pending`，以空 `session_key` 自动创建新会话，并对同一 Judge prompt 重试一次。新会话仍为空或返回运行时错误时立即失败，由中心按 `steward_judge_failed` 留痕；每次请求最多重建一次，禁止形成无限重试。

### 8.9 值守 Codex Agent 独立中转绑定（2.1.101）

值守 Agent 不应无条件继承本机全局 Codex provider。Edge 从 Agent `config.codex_model_provider` 读取 provider，并在新建、恢复专属 Codex 会话时以 `-c model_provider=...` 注入。该覆盖只接受长度 1 至 64 的字母、数字、点、下划线和连字符，非法值忽略并记录告警。由此 Agent 7 可固定使用 `OpenAI3`，即使全局 provider 指向其他中转也不会误路由。
...
中心 judge 只生成回复，不直接调用同步 Bridge continue。编排器创建幂等的 `session.continue.requested`，Edge mailbox 串行执行原 Worker 会话并在 CLI 完成后 ACK。中心 ACK 回调写入 `intervention_completed` 并关闭 watch event；死信写失败审计。消息、attempt、completed 和 watch event 共享 `message_id/correlation_id`。

### 9.2 结构化代答决策与安全升级（2.1.85）

judge 以 `action/reply/reason/risk` JSON 返回决策。`reply` 和 `ask_worker` 只在 normal 风险时可进入可靠 mailbox；`escalate_human` 或 high/critical 保留 visible 事件。Center 在 judge 之外复核删除/破坏、生产变更、提权和凭据处理等问询，即使 judge 误返回 reply 也不得自动发送。旧值守自定义 prompt 的纯文本回复保持向前兼容。

### 9.3 活动事件去重与 Edge provision 串行（2.1.86）

编排器在每次 judge 前使用 binding/session/fingerprint 查询 pending/visible/claimed 事件，命中后直接结束本轮，避免转人工问句每 60 秒重复消耗模型。Edge 自动 provision 不再通过外层 serialized scheduler 包装本身已串行的 provision，避免同 Agent key 嵌套等待。
...
| V4.7 | 2026-07-24 | 2.1.73：历史 Work 镜像 ID 在 Bridge 重连后按 client/local ID 映射到实时索引，避免旧详情继续读取云端旧表 |
| V4.8 | 2026-07-24 | 2.1.74：Agent 主详情复用统一身份解析，页面头部和资源标签保持同一 Bridge 权威身份 |
| V4.9 | 2026-08-03 | 2.1.98：值守记忆默认自动生效，支持粘性停用、恢复和重复学习防复活 |
| V5.0 | 2026-08-03 | 2.1.99：监督调度仅认实时 Bridge 在线，Center 禁止读取本地记忆路径 |
| V5.1 | 2026-08-07 | 2.1.100：Judge 空回复时自动失效损坏的专属会话并有界重建一次 |
