...
export async function POST(
  ...
  const { connection_id, token_usage, status, last_activity } = body;
  const db = getDatabase();
  const now = Math.floor(Date.now() / 1000);
  const workspaceId = auth.user.workspace_id ?? 1;

  // Resolve agent for name-based lookup if needed
  const resolvedParams = await params;
  const agentId = resolvedParams.id;
  let agent: any;
  if (isNaN(Number(agentId))) {
  ...
  if (agent) {
    // Update agent status if provided, otherwise standard heartbeat update
    const newStatus = status || 'idle';
    const newActivity = last_activity || 'Heartbeat (POST)';
    db_helpers.updateAgentStatus(agent.name, newStatus, newActivity, workspaceId);
  }
