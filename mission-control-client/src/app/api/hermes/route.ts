...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (action === 'install-hook') {
      ...
      logger.info('Installed E-Agent-Client hook for Hermes Agent')
    ...
    if (action === 'uninstall-hook') {
      ...
      logger.info('Uninstalled E-Agent-Client hook for Hermes Agent')
...
description: Reports agent telemetry to E-Agent-Client
version: "1.0"
events:
  - agent:start
  - agent:end
