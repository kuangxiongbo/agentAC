...
export default function LoginPage() {
  ...
  // Check if setup is needed or if already authenticated (auth bypass)
  useEffect(() => {
    Promise.all([
      fetch('/api/setup').then(res => res.json().catch(() => ({}))),
      fetch('/api/auth/me').then(res => res.ok ? res.json().catch(() => ({})) : {})
    ]).then(([setupData, authData]) => {
