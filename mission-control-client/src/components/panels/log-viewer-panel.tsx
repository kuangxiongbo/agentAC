...
export function LogViewerPanel() {
  ...
  const loadLogs = useCallback(async (tail = false) => {
    ...
    try {
      ...
      if (data.logs && data.logs.length > 0) {
        if (tail) {
          addLogs([...data.logs].reverse())
          log.debug(`Batch added ${data.logs.length} new logs (tail mode)`)
        } else {
          setLogs([...data.logs].reverse())
          log.debug(`Successfully set ${data.logs.length} logs to store`)
        }
      } else {
  ...
  return (
    <div className="flex flex-col h-full p-4 space-y-4">
      {/* Removed header title and description for a more minimalist UI */}
      ...
      {/* Removed stats and status bar for minimalist UI */}
