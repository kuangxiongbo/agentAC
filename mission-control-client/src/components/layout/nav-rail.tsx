...
const navGroups: NavGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'human-watch', label: 'Human Watch', icon: <ApprovalsIcon />, priority: false, essential: false },
...
export function NavRail() {
  ...
  // Combine all nav array transformations inside a single stable useMemo block
  const { filteredGroups, filteredAllNavItems } = useMemo(() => {
    ...
    const mGroups = navGroups.map(g => {
      const pluginItems = getPluginNavItems()
        .filter(pi => pi.groupId === g.id)
        .map(pi => ({
          id: pi.id,
          label: pi.label,
          icon: pi.icon ? <span key={`icon-${pi.id}`}>{pi.icon}</span> : <PluginIcon key={`icon-plugin-${pi.id}`} />,
          priority: false,
        } as NavItem))
    ...
    const fGroups = mGroups
      .map(g => ({ ...g, items: filterItems(g.items) }))
      .filter(g => g.items.length > 0)

    return {
      filteredGroups: fGroups,
      filteredAllNavItems: fGroups.flatMap(g => flattenItems(g.items)),
    }
  }, [isAdmin, isLocal, isEssential, tn])
