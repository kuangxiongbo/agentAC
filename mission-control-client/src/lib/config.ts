...
// ---------------------------------------------------------------------------
// Auth configuration
// MC_DISABLE_AUTH=true  → skip all authentication (default: true for local mode)
// MC_DISABLE_AUTH=false → enable full session/API-key authentication
// ---------------------------------------------------------------------------
const authDisabledRaw = process.env.MC_DISABLE_AUTH
// Default to disabled (no-auth) when the env var is absent — local-client mode
export const AUTH_DISABLED: boolean =
  authDisabledRaw === undefined ? true : authDisabledRaw !== 'false' && authDisabledRaw !== '0'
