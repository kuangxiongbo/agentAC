...
export function validateDownloadedSkillContent(value: unknown): string {
  if (typeof value !== 'string') throw new Error('Registry returned non-text content')
  if (!value.trim()) throw new Error('Registry returned empty content')
  if (Buffer.byteLength(value, 'utf8') > MAX_REGISTRY_SKILL_BYTES) {
