...
describe('strict workspace resource ownership', () => {
  it('allows owned resources', () => {
    expect(denyResourceOutsideWorkspace(strictUser, 'local_sessions', '/continue', { clientId: 'edge-owned', localAgentId: 7, sessionId: 'session-1', sessionKind: 'codex-cli' })).toBeNull()
    expect(denyResourceOutsideWorkspace(strictUser, 'local_sessions', '/continue', { localAgentId: 9 })).toBeNull()
  })
  it('denies cross-workspace resources', () => {
    expect(denyResourceOutsideWorkspace(strictUser, 'local_sessions', '/continue', { clientId: 'edge-other', sessionId: 'session-1', sessionKind: 'codex-cli' })?.status).toBe(403)
    expect(denyResourceOutsideWorkspace(strictUser, 'gateway_sessions', '/sessions')?.status).toBe(403)
  })
})
