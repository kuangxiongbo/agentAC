...
describe('codex-mcp-injection', () => {
  ...
  it('binds supervised completion identity to the managed process environment', () => {
    ...
    const taskRead = script.slice(
      script.indexOf("name: 'mc_get_task'"),
      script.indexOf("name: 'mc_get_supervision_goal'"),
    )
    ...
    const sessionReads = script.slice(script.indexOf("name: 'mc_list_sessions'"))
    expect(sessionReads).toContain('readOnlyHint: true')
    expect(sessionReads.slice(sessionReads.indexOf("name: 'mc_session_transcript'"))).toContain('readOnlyHint: true')
