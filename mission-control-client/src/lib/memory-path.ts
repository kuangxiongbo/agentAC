...
export const MEMORY_PATH = memoryPathForMode(config.memoryDir, false)
...
export function canonicalizeMemoryRelativePath(value: unknown): string {
  ...
  if (!input || input.length > 1024 || input.includes('\0')) throw new Error('Invalid memory path')
...
export function isPathAllowed(relativePath: string): boolean {
  ...
  try { normalized = canonicalizeMemoryRelativePath(relativePath) } catch { return false }
