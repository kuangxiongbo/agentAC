...
export function useWebSocket() {
  ...
  const getGatewayErrorHelp = useCallback((message: string): string => {
    ...
    if (
      ...
      return 'Gateway requires device identity. Open E-Agent-Client via HTTPS (or localhost), then reconnect so WebCrypto signing can run.'
    }
  ...
  const connect = useCallback((url: string, token?: string) => {
    ...
    try {
      ...
      ws.onerror = (error) => {
        ...
        log.warn('WebSocket error (gateway may be offline)')
      ...
      log.warn('Failed to connect to WebSocket, gateway may be offline')
