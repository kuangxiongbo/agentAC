...
function isGatewayAvailable(): boolean {
  // Gateway is available only if:
  // 1. The openclaw binary exists at its configured path, OR
  // 2. A gateway is registered in the DB (external gateway mode)
  //
  // NOTE: We do NOT treat openclawHome alone as proof — the directory may
  // exist from a previous install while the binary is gone.
  try {
    const { existsSync } = require('fs')
    const { which } = require('child_process')
    // Check if the openclaw binary is on PATH or at the configured bin path
    const bin = config.openclawBin || 'openclaw'
    const isAbsolutePath = bin.startsWith('/') || bin.startsWith('~')
    if (isAbsolutePath) {
      if (existsSync(bin)) return true
    } else {
      // Try to resolve from PATH using 'which'
      try {
        const { execSync } = require('child_process')
        execSync(`which ${bin} 2>/dev/null`, { timeout: 1000 })
        return true
      } catch {
        // Binary not found on PATH — fall through to DB check
      }
    }
  } catch { /* fs not available in edge runtime */ }

  // Fallback: check if any gateway is registered in the DB
  try {
    const db = getDatabase()
    const row = db.prepare('SELECT COUNT(*) as c FROM gateways WHERE status != \'offline\'').get() as { c: number } | undefined
    return (row?.c ?? 0) > 0
  } catch {
    return false
  }
...
export async function dispatchAssignedTasks(): Promise<{ ok: boolean; message: string }> {
  ...
  for (const task of tasks) {
    ...
    try {
      ...
      // Notify remote server of task completion (non-blocking)
      if ((existingMeta as any).remote_task_id) {
        sendBridgeEvent('task_status', {
          taskId: (existingMeta as any).remote_task_id,
          localTaskId: task.id,
          status: 'completed',
