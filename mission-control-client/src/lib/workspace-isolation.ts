...
export interface EdgeResourceOwnership { clientId?: string | null; localAgentId?: number | null; sessionId?: string | null; sessionKind?: string | null }

function readIsolation(user: User): IsolationRecord | null {
  return getDatabase().prepare(`SELECT id, tenant_id, isolation FROM workspaces WHERE id = ? AND tenant_id = ? LIMIT 1`)
    .get(user.workspace_id, user.tenant_id) as IsolationRecord | undefined || null
}
export function getWorkspaceIsolation(user: User): WorkspaceIsolation | null { return readIsolation(user)?.isolation ?? null }
export function resolveSharedRuntimeWorkspaceId(): number | null {
  const rows = getDatabase().prepare(`SELECT id FROM workspaces WHERE isolation = 'shared' ORDER BY id LIMIT 2`).all() as Array<{ id: number }>
  return rows.length === 1 ? rows[0].id : null
}
function auditDenial(user: User, resource: WorkspaceResource, route: string, reason: string): void {
  try {
    logAuditEvent({ action: 'workspace_isolation_denied', actor: user.username || 'unknown', actor_id: user.id,
...
function edgeOwnershipMatches(user: User, ownership: EdgeResourceOwnership): boolean {
  ...
  if (!ownership.clientId) {
    ...
    return Boolean(db.prepare(`SELECT 1 FROM agents WHERE id = ? AND workspace_id = ? LIMIT 1`).get(ownership.localAgentId, user.workspace_id))
  }
  ...
  if (ownership.localAgentId != null && !db.prepare(`SELECT 1 FROM sync_agent_index WHERE client_id = ? AND local_agent_id = ? LIMIT 1`).get(ownership.clientId, ownership.localAgentId)) return false
  if (ownership.sessionId || ownership.sessionKind) {
    if (!ownership.sessionId || !ownership.sessionKind) return false
    if (!db.prepare(`SELECT 1 FROM sync_sessions WHERE client_id = ? AND session_id = ? AND session_kind = ? LIMIT 1`).get(ownership.clientId, ownership.sessionId, ownership.sessionKind)) return false
  }
  return true
}
export function denyResourceOutsideWorkspace(user: User, resource: WorkspaceResource, route: string, ownership?: EdgeResourceOwnership | null): NextResponse | null {
  const workspace = readIsolation(user)
  if (!workspace) { auditDenial(user, resource, route, 'workspace_context_unavailable'); return NextResponse.json({ error: 'Workspace isolation context is unavailable' }, { status: 403 }) }
  if (ownership) {
...
export function resolveWorkspaceMemoryAccess(user: User) {
  ...
  if (workspace.isolation === 'strict') return { isolation: 'strict' as const, root: join(dirname(base), `${basename(base)}-workspaces`, String(workspace.id)), scope: `workspace:${workspace.id}` }
