...
export function getUserFromRequest(request: Request): User | null {
  ...
  // No-auth mode: return a synthetic admin user when authentication is disabled
  // Controlled by MC_DISABLE_AUTH env var (default: true for local-client mode)
  if (AUTH_DISABLED) {
    return {
      id: 1,
...
export function requireRole(
  ...
  const user = getUserFromRequest(request)
  if (!user) {
    // If auth is disabled (MC_DISABLE_AUTH), getUserFromRequest already returns a synthetic
    // admin user. We only reach here when auth IS enabled and no credentials were provided.
